...
export class b2WheelJoint extends b2Joint
{
	...
	public InitVelocityConstraints(data)
	{
		...
		if (this.m_frequencyHz > 0)
		{
			...
			var invMass: number = mA + mB + iA * this.m_sAx * this.m_sAx + iB * this.m_sBx * this.m_sBx;

			if (invMass > 0)
			{
				this.m_springMass = 1 / invMass;

				var C: number = b2DotVV(d, this.m_ax);

				// Frequency
				var omega: number = 2 * b2_pi * this.m_frequencyHz;

				// Damping coefficient
				var dc: number = 2 * this.m_springMass * this.m_dampingRatio * omega;

				// Spring stiffness
				var k: number = this.m_springMass * omega * omega;
		...
		if (data.step.warmStarting)
		{
			...
			var LB: number = this.m_impulse * this.m_sBy + this.m_springImpulse * this.m_sBx + this.m_motorImpulse;
	...
	public SolveVelocityConstraints(data)
	{
		...
		if (true)
		{
			var Cdot: number = b2DotVV(this.m_ax, b2SubVV(vB, vA, b2Vec2.s_t0)) + this.m_sBx * wB - this.m_sAx * wA;
			var impulse: number = -this.m_springMass * (Cdot + this.m_bias + this.m_gamma * this.m_springImpulse);
			this.m_springImpulse += impulse;

			//b2Vec2 P = impulse * m_ax;
			var P: b2Vec2 = b2MulSV(impulse, this.m_ax, b2WheelJoint.SolveVelocityConstraints_s_P);
			var LA: number = impulse * this.m_sAx;
			var LB: number = impulse * this.m_sBx;
		...
		if (true)
		{
			var Cdot: number = b2DotVV(this.m_ay, b2SubVV(vB, vA, b2Vec2.s_t0)) + this.m_sBy * wB - this.m_sAy * wA;
			var impulse: number = -this.m_mass * Cdot;
			this.m_impulse += impulse;

			//b2Vec2 P = impulse * m_ay;
			var P: b2Vec2 = b2MulSV(impulse, this.m_ay, b2WheelJoint.SolveVelocityConstraints_s_P);
			var LA: number = impulse * this.m_sAy;
			var LB: number = impulse * this.m_sBy;
	...
	public SolvePositionConstraints(data)
	{
		...
		var ay: b2Vec2 = b2MulRV(qA, this.m_localYAxisA, this.m_ay);

		//float32 sAy = b2Cross(d + rA, ay);
		var sAy = b2CrossVV(b2AddVV(d, rA, b2Vec2.s_t0), ay);
		//float32 sBy = b2Cross(rB, ay);
		var sBy = b2CrossVV(rB, ay);

		//float32 C = b2Dot(d, ay);
		var C: number = b2DotVV(d, this.m_ay);

		var k: number = this.m_invMassA + this.m_invMassB + this.m_invIA * this.m_sAy * this.m_sAy + this.m_invIB * this.m_sBy * this.m_sBy;
		...
		//b2Vec2 P = impulse * ay;
		var P: b2Vec2 = b2MulSV(impulse, ay, b2WheelJoint.SolvePositionConstraints_s_P);
	...
	public GetJointTranslation(): number
	{
		...
		var pA: b2Vec2 = bA.GetWorldPoint(this.m_localAnchorA, new b2Vec2());
		var pB: b2Vec2 = bB.GetWorldPoint(this.m_localAnchorB, new b2Vec2());
		var d: b2Vec2 = b2SubVV(pB, pA, new b2Vec2());
		var axis: b2Vec2 = bA.GetWorldVector(this.m_localXAxisA, new b2Vec2());
	...
	{
		var wA: number = this.m_bodyA.m_angularVelocity;
		var wB: number = this.m_bodyB.m_angularVelocity;
		return wB - wA;
	}
