...
export class b2MotorJointDef extends b2JointDef
{
	...
	public Initialize(bA, bB)
	{
		...
		var angleA: number = this.bodyA.GetAngleRadians();
		var angleB: number = this.bodyB.GetAngleRadians();
...
export class b2MotorJoint extends b2Joint
{
	...
	public SolveVelocityConstraints(data)
	{
		...
		var inv_h: number = data.step.inv_dt;

		// Solve angular friction
		{
			var Cdot: number = wB - wA + inv_h * this.m_correctionFactor * this.m_angularError;
		...
		// Solve linear friction
		{
			...
			var impulse_v2: b2Vec2 = b2MulMV(this.m_linearMass, Cdot_v2, b2MotorJoint.SolveVelocityConstraints_s_impulse_v2).SelfNeg();
	...
	public Dump()
	{
		if (DEBUG)
		{
			...
			b2Log("  var jd: b2MotorJointDef = new b2MotorJointDef();\n");
