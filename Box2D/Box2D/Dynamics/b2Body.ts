...
export class b2Body
{
	...
	public CreateFixture(def: b2FixtureDef): b2Fixture
	{
		...
		{
			var broadPhase: b2BroadPhase = this.m_world.m_contactManager.m_broadPhase;
	...
	{
		var def: b2FixtureDef = b2Body.CreateFixture2_s_def;
		def.shape = shape;
		def.density = density;
	...
	public SetMassData(massData: b2MassData): void
	{
		...
		// Move center of mass.
		var oldCenter: b2Vec2 = b2Body.SetMassData_s_oldCenter.Copy(this.m_sweep.c);
	...
	public ResetMassData(): void
	{
		...
		var localCenter: b2Vec2 = b2Body.ResetMassData_s_localCenter.SetZero();
		for (var f = this.m_fixtureList; f; f = f.m_next)
		{
			if (f.m_density == 0)
			{
			...
			var massData: b2MassData = f.GetMassData(b2Body.ResetMassData_s_massData);
			this.m_mass += massData.mass;
		...
		// Move center of mass.
		var oldCenter: b2Vec2 = b2Body.ResetMassData_s_oldCenter.Copy(this.m_sweep.c);
	...
	public SetType(type: b2BodyType): void
	{
		...
		{
			var proxyCount: number = f.m_proxyCount;
			for (var i: number = 0; i < proxyCount; ++i)
			{
	...
	{
		var status: boolean = (this.m_flags & b2BodyFlag.e_fixedRotationFlag) == b2BodyFlag.e_fixedRotationFlag;
		if (status == flag)
		{
			return;
		}
	...
	{
		var xf1: b2Transform = b2Body.SynchronizeFixtures_s_xf1;
